...
The DAG uses a layered (dagre) layout, configured with `dag_layout()`. It flows
top-to-bottom by default. On large, wide boards a top-down flow collides sibling
labels (blockr nodes label below the icon); switching to `rankdir = "LR"` there
spreads siblings vertically and keeps the graph compact. Pass a `dag_layout()`
to `layout` to change the direction or spacing; the flow direction also sets the
port sides and edge curve:
...
serve(
  new_dock_board(
    ...
    links = list(from = "a", to = "b", input = "data"),
    extensions = new_dag_extension(
      layout = dag_layout(rankdir = "LR")
    )
  )
)
```

The chosen layout is persisted across save / restore.
