...
- The node status badge reflects a block's full eval status, not just error conditions ([#145](https://github.com/BristolMyersSquibb/blockr.dag/issues/145)). It consumes `board$eval[[id]]` together with `blockr.dock::block_status_style()` -- the colour / size / placement spec both front-ends share -- so a node shows an amber `waiting`, yellow `unset` or red `failed` dot matching the dock's on-icon status dot. Any error condition still shows red (a render-phase error leaves a block `ready` but errored); `ready` and `dormant` nodes carry no badge. Hovering a badged node shows the status reason as a tooltip (e.g. "Waiting for a data input").
